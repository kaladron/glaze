...
suite char_array = [] {
   ...
   "char array with leading zero bug"_test = [] {
      // Regression test for char array serialization bug similar to #1956
      // When char foo[4] = {0, 0, 1, 0}, all data is lost due to checking *value.data()
      char arr[4] = {0, 0, 1, 0};
   ...
   "char array comparison with int array"_test = [] {
...
suite char_array_tests = [] {
   ...
   "byte_array wrapper for fixed-size serialization"_test = [] {
      // NEW: byte_array wrapper for fixed-size byte array behavior
      char arr[4] = {0, 0, 1, 0};
      ...
      // NEW behavior: all bytes serialized with proper escaping
      expect(json_output == "\"\\u0000\\u0000\\u0001\\u0000\"") << "Got: " << json_output;
   };
